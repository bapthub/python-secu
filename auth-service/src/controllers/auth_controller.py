...
def signup():
    ...
    if not validate_password(password):
        return Response(json.dumps({'error': 'password do not match required statements'}), status=400)
    ...
    if existing_email:
        return Response(json.dumps({'error': 'email déjà utilisé'}), status=400)
    else:
...
def code():
    ...
    if verify_mail(email, cryptomail_collection, code):
        ...
        return Response(json.dumps({'status':'Code valide.'}), status=200)
    else:
        return Response(json.dumps({'error': 'Email ou code incorrect.'}), status=400)
...
def resend():
    ...
    if existing_mail:
        ...
        if status == "Active":
            # flash("", "info")
            return Response(json.dumps({'status':'Compte déjà activé.'}),status=400)

        send_mail(email, cryptomail_collection)
        return Response(json.dumps({'status': 'Code envoyé.'}),status=200)
    return Response(json.dumps({'error': 'email inconnu'}),status=400)
...
def certificate():
    ...
    if existing_mail:
        ...
        if check_password_hashed(user_pass, password):
            ...
            return Response(json.dumps({'error': 'Utilisateur ou mot de passe incorrect.'}),status=400)
    else:
        return Response(json.dumps({'error': 'Utilisateur ou mot de passe incorrect.'}),status=400)
        ...
        if existing_mail:
            ...
            if check_password_hashed(user_pass, password):
                ...
                if existing_mail.get("status") == "Inactive":
                    return Response(json.dumps({'error':'Compte inactif'}),status=400)
                ...
                if res != "valid":
                    return Response(json.dumps({'error':'Certificat invalide.'}),status=400)
                if os.path.exists(file_loc):
                    os.remove(file_loc)
                jwt_token = encode_jwt(2, {"email": email})
                return Response(
                    json.dumps({
                        'status': 'Authentification terminée',
            ...
            return Response(json.dumps({'error':'Utilisateur ou mot de passe incorrect'}),status=400)
    return Response(json.dumps({'error':'Données incorrectes'}),status=400)
...
def login_jwt():
    ...
    if return_val == True:
        return Response(json.dumps({'status': 'Token Valide, authentification réussie'}), status=200)
    else:
        return Response(json.dumps({'error': return_val }), status=400)